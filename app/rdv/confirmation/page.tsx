...
import { sendBookingEmail } from "../../actions/sendMessage";
...
function ConfirmationPageContent() {
  ...
  const executeTurnstileRef = useRef<(() => void) | null>(null);
  ...
  const sendBookingRequest = useCallback(
    async (token: string) => {
      ...
      formData.append("service", service || "Rendez-vous");
      formData.append("date", dateISO);
      formData.append("time", time);
      formData.append("notes", `Demande via la page de confirmation. Téléphone : ${mailPhone}`);
    ...
    [dateISO, email, mailName, mailPhone, phone, sendBookingEmail, service, time]
  );

  const handleTurnstileToken = useCallback(
    (token: string) => {
      setTurnstileToken(token);
      if (token && pendingSendRef.current) {
        pendingSendRef.current = false;
        sendBookingRequest(token);
      }
    },
    [sendBookingRequest]
  );
  ...
  async function handleSend() {
    ...
    if (!turnstileToken) {
      ...
      executeTurnstileRef.current?.();
      return;
    }
    await sendBookingRequest(turnstileToken);
  }

  const canSend = isComplete && sendStatus !== "sending";
            ...
            {isComplete && (
              <div className="sr-only" aria-hidden="true">
                <TurnstileWidget
                  ...
                  size="invisible"
                  onReady={({ execute }) => {
                    executeTurnstileRef.current = execute;
                  }}
                />
              </div>
            )}
